...
1. **Match path** — when the recall pipeline surfaces at least one memory,
   ``memories`` returns the Claude Code UserPromptSubmit hook envelope
   verbatim::
...
2. **No-op path** — when chat extracts no queries, the tool returns a
   true empty MCP response: ``content == []`` and ``structured_content
   is None``. Load-bearing contract for the fix tracked at
   Pondsiders/Alpha#24: anything else (even ``{"result": null}``, which
   is what ``dict | None`` returns produce via FastMCP's primitive
   wrapping) triggers Claude Code's fallback "hook success: completed"
   status-string injection.
...
async def test_memories_match_returns_user_prompt_submit_envelope(
    ...
    """memories() returns a UserPromptSubmit hook envelope with formatted memory blocks."""
